...
def send_sales_order_document(docname):
    """
    Main function to send Sales Order PDF via WhatsApp.
    """
    try:
    ...
    to_whatsapp_raw = customer_doc.whatsapp_number

    if not to_whatsapp_raw:
    ...
    raw_number = ''.join(filter(str.isdigit, to_whatsapp_raw))
    ...
    # Final validation: must be 12 digits starting with 256
    ...
    payload = {
        ...
        "document": {
            "id": media_id,
            "filename": filename,
            "caption": f"Your Sales Order from {doc.company}\nOrder Number: {doc.name}"
        }
    }
    ...
    try:
        ...
        doc.add_comment("Comment", f"WhatsApp Sales Order PDF sent successfully to {to_whatsapp}")
...
def send_sales_order_background(doc, method):
    """
    Hook: on_submit of Sales Order → enqueue WhatsApp send
    """
    if not doc.customer:
